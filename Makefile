...
EXECUTABLE     := $(CONAN_BUILD_DIR)/bin/gstreamer_demo
...
help:
	@echo "$(CYAN)╔════════════════════════════════════════════╗$(NC)"
	@echo "$(CYAN)║      GStreamer Demo - Build System       ║$(NC)"
	@echo "$(CYAN)╚════════════════════════════════════════════╝$(NC)"
	@echo ""
	...
	@echo "  make all && make run ARGS=file:///tmp/video.mp4"
	@echo "  make run ARGS=https://www.freedesktop.org/software/gstreamer-sdk/data/media/sintel_trailer-480p.webm"
	@echo ""
	@echo "$(RED)⚠️  ATENÇÃO:$(NC)"
	@echo "  GStreamer é grande! Primeira compilação: 10-20 minutos"
	@echo "  Proximas vezes: instantâneo (cache)"

init:
	@echo "$(BLUE)>>> 📦 Instalando GStreamer com Conan...$(NC)"
	@echo "$(YELLOW)    Isso pode demorar 10-20 min na 1ª vez...$(NC)"
...
run:
	...
	@echo "$(CYAN)>>> ▶️  Executando gstreamer_demo $(ARGS)$(NC)"
	$(EXECUTABLE) $(ARGS)
...
info:
	...
	@if [ -f "$(EXECUTABLE)" ]; then ls -lh $(EXECUTABLE); fi.PHONY: all clean init config build run rebuild help info
